...
let newCardss = "";
wconstmeals.forEach(loopCardss);
function loopCardss(value) {
    newCardss +=    `
        <div class="card">
        <img src="${value.strCategoryThumb}" alt="">
        <b class="class">${value.idCategory}</b>
        <b class="card-title">${value.strCategory}</b>
        <p class="card-description">${value.strCategoryDescription}</p>
        </div>
    `;
}
document.getElementById('card').innerHTML = newCardss;